...
class DAVISEvaluation(object):
    ...
    def _evaluate_semisupervised(all_gt_masks, all_res_masks, all_void_masks, metric):
        if all_res_masks.shape[0] > all_gt_masks.shape[0]:
            ...
            # Missing objects in prediction only in case where
            # num_objects in masks < num_objects in GT, an num_objects
            # takes max so the missing objects will always be the last
            # -> Zero padding to the last is okay.
            zero_padding = np.zeros((all_gt_masks.shape[0] - all_res_masks.shape[0], *all_res_masks.shape[1:]))
            all_res_masks = np.concatenate([all_res_masks, zero_padding], axis=0)

        # Collect J and F over objects and frames
        j_metrics_res, f_metrics_res = np.zeros(all_gt_masks.shape[:2]), np.zeros(all_gt_masks.shape[:2])

        for ii in range(all_gt_masks.shape[0]):
            # Loop over objects
            # Input tensors: (N_objects, N_frames, H, W)
            if 'J' in metric:
    ...
    def evaluate(self, res_path, metric=('J', 'F'), debug=False):
        ...
        for seq in tqdm(list(self.dataset.get_sequences())):
            # all_gt_masks: (#_object, #_frames, )
            all_gt_masks, all_void_masks, all_masks_id, obj_first_dict = self.dataset.get_all_masks(seq, True)
            if self.task == 'semi-supervised':
                # For DAVIS, remove the first and last mask and consider inbetween ones
                # all_gt_masks, all_masks_id = all_gt_masks[:, 1:-1, :, :], all_masks_id[1:-1]
                # For VISOR, should consider frames that not in the annotation
                all_gt_masks, all_masks_id = all_gt_masks[:, :, :, :], all_masks_id[:]
            ...
            for ii in range(all_gt_masks.shape[0]):  # Loop over objects
                seq_name = f'{seq}_{ii+1}'
                # NOTE: modification, only use results from the first frame an object appeared
                # till the end of sequence (similar to Youtube-VOS style)

                if 'J' in metric:
                    [JM, JR, JD] = utils.db_statistics(j_metrics_res[ii, obj_first_dict[ii + 1]:])
                    metrics_res['J']["M"].append(JM)
                    metrics_res['J']["R"].append(JR)
                    metrics_res['J']["D"].append(JD)
                    metrics_res['J']["M_per_object"][seq_name] = JM
                if 'F' in metric:
                    [FM, FR, FD] = utils.db_statistics(f_metrics_res[ii, obj_first_dict[ii + 1]:])
